...
void inicializarLoja (Supermercado *sm)
{
    ...
    for (int i = 0; i < sm->config.n_caixas; i++)
    {
        sm->caixas[i].id    = i + 1;
        sm->caixas[i].ativa = 0;
        sm->caixas[i].seg_fim_atendimento = -1;// segundos simulados em que o cliente termina a compra (-1 porque ainda ninguem comprou nada)
    }
    carregarBanidos(sm);
    iniciarDia(sm);
}
        ...
        if (nodo->id_cliente == id)
        {
            ...
            if (f_leitura && f_escrita)
            {
                ...
                while (fgets(linha, sizeof(linha), f_leitura))
                {
                    ...
                    // usa sscanf para extrair o ID da linha corretamente
                    int id_linha = 0;
...
void processarAtendimento (Supermercado *sm)
{
    ...
    for(int i = 0; i< sm->config.n_caixas; i++)
    {
        ...
        if(seg_atual >= cai->seg_fim_atendimento)
        {
            ...
            registar_cliente_atendido(sm, cai, atendido);
...
// conta a quantidade de pessoas nas filas
